...
class Proto {
public:
  static constexpr const char *TickerName = "NMC";
  ...
  using Block = BTC::Proto::BlockTy<NMC::Proto>;
...
public:
  ...
  using NmcWork = BTC::WorkTy<NMC::Proto, BTC::Stratum::HeaderBuilder, BTC::Stratum::CoinbaseBuilder, BTC::Stratum::Notify, BTC::Stratum::Prepare>;
  ...
  public:
    ...
    virtual std::string blockHash(size_t workIdx) override {
      ...
      else if (workIdx - 1 < NMCHeader_.size())
        return NMCHeader_[workIdx-1].hash().getHexLE();
      else
        return std::string();
    }
    ...
    virtual void buildBlock(size_t workIdx, xmstream &blockHexData) override {
      ...
      } else if (nmcWork(workIdx - 1)) {
        nmcWork(workIdx - 1)->buildBlockImpl(NMCHeader_[workIdx-1], NMCWitness_[workIdx-1], blockHexData);
      }
    }

    virtual CCheckStatus checkConsensus(size_t workIdx, const UInt<256> &shareTarget) override {
      if (workIdx == 0 && btcWork())
        return BTC::Stratum::Work::checkConsensusImpl(BTCHeader_, NMCConsensusCtx_, shareTarget);
      else if (nmcWork(workIdx - 1))
        return NMC::Stratum::NmcWork::checkConsensusImpl(NMCHeader_[workIdx - 1], BTCConsensusCtx_, shareTarget);
      return CCheckStatus();
    }

  private:
    BTC::Stratum::Work *btcWork() { return static_cast<BTC::Stratum::Work*>(Works_[0].Work); }
    NMC::Stratum::NmcWork *nmcWork(unsigned index) { return static_cast<NMC::Stratum::NmcWork*>(Works_[index + 1].Work); }
  ...
  private:
    ...
    std::vector<NMC::Proto::BlockHeader> NMCHeader_;
    std::vector<BTC::CoinbaseTx> NMCLegacy_;
    std::vector<BTC::CoinbaseTx> NMCWitness_;
    std::vector<BaseBlob<256>> NMCHeaderHashes_;
    std::vector<int> NMCWorkMap_;
    NMC::Proto::CheckConsensusCtx NMCConsensusCtx_;
  };
  ...
  static NmcWork *newSecondaryWork(int64_t stratumId,
                                   PoolBackend *backend,
                                   size_t backendIdx,
                                   const CMiningConfig &miningCfg,
                                   const std::vector<uint8_t> &miningAddress,
    ...
    std::unique_ptr<NmcWork> work(new NmcWork(stratumId,
                                              blockTemplate.UniqueWorkId,
                                              backend,
                                              backendIdx,
                                              miningCfg,
...
struct X {
  using Proto = NMC::Proto;
  using Stratum = NMC::Stratum;
...
// Header
namespace BTC {
template<> struct Io<NMC::Proto::BlockHeader> {
  static void serialize(xmstream &dst, const NMC::Proto::BlockHeader &data);
  static void unserialize(xmstream &src, NMC::Proto::BlockHeader &data);
};
}
