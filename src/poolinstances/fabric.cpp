...
std::unordered_map<std::string, PoolInstanceFabric::NewPoolInstanceFunction> PoolInstanceFabric::FabricData_ = {
  ...
  {"FB.stratum", [](asyncBase *base, UserManager &userMgr, const std::vector<PoolBackend*> &linkedBackends, CThreadPool &pool, unsigned instanceId, unsigned instancesNum, rapidjson::Value &config, CPriceFetcher *priceFetcher) {  return StratumInstance::newInstance<FB::Stratum>(base, userMgr, linkedBackends, pool, instanceId, instancesNum, config, priceFetcher);}},
