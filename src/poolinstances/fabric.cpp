...
std::unordered_map<std::string, PoolInstanceFabric::NewPoolInstanceFunction> PoolInstanceFabric::FabricData_ = {
  ...
  { "FB.stratum", [](asyncBase *base, UserManager &userMgr,
                    ...
                    CThreadPool &pool, unsigned instanceId,
                    unsigned instancesNum, rapidjson::Value &config,
                    CPriceFetcher *priceFetcher) {
      return new StratumInstance<FB::X>(
        base, userMgr, linkedBackends,
        pool, instanceId, instancesNum,
        config, priceFetcher);
}},
